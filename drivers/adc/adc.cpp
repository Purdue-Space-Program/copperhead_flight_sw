...
etl::expected<void, ADC_Base_Driver::ErrorCode> ADC_MCU_Driver::init(void)
{
    ...
    if (SPI_WRITE(&cmd, 1) != 0)
        return etl::unexpected(ErrorCode::ADS_WSPI_ERR);
    return {};
}
...
etl::expected<void, ADC_Base_Driver::ErrorCode> ADC_MCU_Driver::read_register(const Register reg, uint8_t *value)
{
    ...
    if (SPI_READ(value, 1) != 0)
        return etl::unexpected(ErrorCode::ADS_RSPI_ERR);
    return {};
}
...
etl::expected<void, ADC_Base_Driver::ErrorCode> ADC_MCU_Driver::read_register_mul(const Register start_reg,
...
etl::expected<void, ADC_Base_Driver::ErrorCode> ADC_MCU_Driver::write_register_mul(Register start_reg, uint8_t *values,
                                                                                   uint8_t size)
{
    if (size == 0)
        return etl::unexpected(ErrorCode::ADS_ERR);
