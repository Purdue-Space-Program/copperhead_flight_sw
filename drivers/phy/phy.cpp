...
etl::expected<void, PHY_Base_Driver::ErrorCode> PHY_MCU_Driver::init(void) {
    if (write_register_bit(PHY_CONTROL_REGISTER, static_cast<uint8_t>(ControlRegister::PHY_CTRL_SOFT_RESET), true) == ErrorCode::PHY_ERR_WRITE_REG)
...
PHY_Base_Driver::ErrorCode PHY_MCU_Driver::write_register_bit(uint8_t reg_addr, uint8_t bit_pos, bool set_bit) {  
    if (bit_pos > 15) return ErrorCode::PHY_ERR_WRITE_REG;
