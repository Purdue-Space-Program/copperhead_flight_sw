...
etl::expected<void, PHY_Base_Driver::ErrorCode> PHY_MCU_Driver::init(void)
{
    if (write_register_bit(PHY_CONTROL_REGISTER, static_cast<uint8_t>(ControlRegister::PHY_CTRL_SOFT_RESET), true) ==
        ErrorCode::PHY_ERR_WRITE_REG)
        return etl::unexpected(PHY_Base_Driver::ErrorCode::PHY_ERR_INIT);
    return {};
}

etl::expected<uint16_t, PHY_Base_Driver::ErrorCode> PHY_MCU_Driver::read_register(const uint8_t reg_addr)
{
    ...
    case PHY_AUTO_NEGOTIATION_REGISTER:
        break;
    default:
        return etl::unexpected(ErrorCode::PHY_ERR_READ_REG);
    }
    uint32_t temp;
...
PHY_Base_Driver::ErrorCode PHY_MCU_Driver::write_register_bit(uint8_t reg_addr, uint8_t bit_pos, bool set_bit)
{
    if (bit_pos > 15)
        return ErrorCode::PHY_ERR_WRITE_REG;
    if (!read_register(reg_addr).has_value())
    {
        return ErrorCode::PHY_ERR_READ_REG;
    }
    ...
    return static_cast<ErrorCode>(mdio_write(MDIO_LAN8742A_PHY_ADDRESS, reg_addr, reg_value));
}

etl::expected<uint16_t, PHY_Base_Driver::ErrorCode> PHY_MCU_Driver::read_status(void)
{
    return read_register(PHY_STATUS_REGISTER);
}

etl::expected<uint16_t, PHY_Base_Driver::ErrorCode> PHY_MCU_Driver::read_mode(void)
{
...
etl::expected<void, PHY_Base_Driver::ErrorCode> PHY_MCU_Driver::apply_mode(uint8_t mode)
{
    const uint16_t regs[] = {PHY_CONTROL_REGISTER,          PHY_CONTROL_REGISTER,
                             PHY_CONTROL_REGISTER,          PHY_CONTROL_REGISTER,
                             PHY_AUTO_NEGOTIATION_REGISTER, PHY_AUTO_NEGOTIATION_REGISTER,
                             PHY_AUTO_NEGOTIATION_REGISTER, PHY_AUTO_NEGOTIATION_REGISTER};
    const uint8_t bits[] = {static_cast<const uint8_t>(ControlRegister::PHY_CTRL_SPEED_SELECT),
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        if (write_register_bit(regs[i], bits[i], bit_val) != ErrorCode::PHY_OK)
        {
            return etl::unexpected(ErrorCode::PHY_ERR_READ_REG);
        }
    }
